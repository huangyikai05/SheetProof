...
def _summary_markdown(
    ...
    lines = [
        ...
        "",
        f"Trusted base `{_markdown(trusted_base[:12])}` Â· "
        f"Merge base `{_markdown(comparison_base[:12])}` Â· "
        f"Head `{_markdown(head[:12])}` Â· "
        f"Compared **{compared}** Â· Blocked **{blocked}** Â· Skipped **{skipped}** Â· "
        f"Unreviewable **{unreviewable}** Â· Errors **{errors}**",
        "",
        config_note,
        "",
    ]
            ...
            lines.append(
                ...
                + " | ".join(
                    ...
                    for value in (
                        ...
                        outcome.workbook,
                        risk,
                        outcome.changed_cells if outcome.changed_cells is not None else "â€”",
                        (
                            outcome.formula_overwrites
                            if outcome.formula_overwrites is not None
                            else "â€”"
                        ),
                        outcome.failed_rules if outcome.failed_rules is not None else "â€”",
                        detail,
                    )
                )
                + " |"
