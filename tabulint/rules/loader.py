"""Load and validate Tabulint's YAML configuration."""
...
from tabulint.models import RuleType, TabulintConfig
from tabulint.rules.builtin_rules import RuleEvaluationError, parse_range
...
def validate_config_text(text: str) -> TabulintConfig:
    ...
    try:
        config = TabulintConfig.model_validate(document)
    except ValidationError as exc:
        detail = _validation_detail(exc)
        raise ConfigurationError(f"Invalid Tabulint configuration: {detail}") from exc
    _validate_semantics(config)
    return config


def load_config(path: str | Path | None = None) -> TabulintConfig:
    """Load a configuration file, or return safe defaults when ``path`` is ``None``."""

    if path is None:
        return TabulintConfig()
...
def _validate_semantics(config: TabulintConfig) -> None:
