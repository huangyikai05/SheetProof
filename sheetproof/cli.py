...
from sheetproof.exceptions import ConfigurationError, SheetProofError
from sheetproof.models import RiskLevel, RuleStatus
from sheetproof.parser.workbook import WorkbookParser
from sheetproof.reports.html_report import write_html_report
from sheetproof.reports.json_report import write_json_report
from sheetproof.rules.loader import load_config
...
rules_app = typer.Typer(help="Validate and work with sheetproof.yml rules.", no_args_is_help=True)
...
def compare_command(
    ...
    config: Annotated[
        Path | None, typer.Option("--config", "-c", help="Path to sheetproof.yml.")
    ] = None,
    ...
    except (SheetProofError, ValidationError, OSError, ValueError) as exc:
        _fail(str(exc), 2)
...
def inspect_command(
    ...
    except (SheetProofError, OSError, ValueError) as exc:
        _fail(str(exc), 2)
    except Exception as exc:
        if debug:
            raise
...
def validate_rules_command(
    config: Annotated[Path, typer.Argument(help="Path to sheetproof.yml.")],
) -> None:
...
def version_command() -> None:
    """Print the installed SheetProof version."""
