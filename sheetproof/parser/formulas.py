...
SheetProof does not try to be an Excel calculation engine.  This module only
...
def qualify_cell(sheet: str, coordinate: str) -> str:
    """Create SheetProof's stable ``Sheet!A1`` location representation."""

    return f"{sheet}!{normalise_coordinate(coordinate)}"


def split_location(location: str, *, default_sheet: str | None = None) -> tuple[str, str]:
    """Split a SheetProof or Excel-like location into sheet and coordinate."""
